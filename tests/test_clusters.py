...
def test_api_create(lavaclient, cluster_create_response):
    with patch.object(lavaclient, '_request') as request:
        request.return_value = cluster_create_response
        resp = lavaclient.clusters.create(
            'cluster_name', 'stack_id')

        assert isinstance(resp, response.ClusterDetail)
        assert resp.service_users
        assert all(isinstance(user, response.ServiceUserDetail)
                   for user in resp.service_users)
